library(here)
cluster_dir <- here("phmrc_adult_5_causes_uncertainty", "cluster_output")
...
posteriors_list <- lapply(1:length(models), function(k) {
    model <- models[k]
    top_btl <- compositional_multi_btl(A_U = top_array[,,k],
                                       ...
                                       epsilon = .01,
                                       alpha = 10)
    comp_btl <- compositional_multi_btl(A_U = comp_array[,,k],
...
csmf_estimates <-
    inner_join(ens_top_comp, true_df, by = "Parameter") %>%
...
posteriors_list[[length(models)+1]] <- csmf_acc_df

csmf_acc_all <- do.call(rbind, posteriors_list)
#csmf_acc_all <- do.call(rbind, lapply(posteriors_list, function(x) x$csmf_acc_df))
csmf_acc_all <- cbind(as.data.frame(csmf_acc_all), setting)
